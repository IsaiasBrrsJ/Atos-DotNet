...
SELECT name
FROM sys.databases;  --mesmo que (show databases)
...
CREATE TABLE Livro (
  ...
  CONSTRAINT fk_Livro_Editora --restricao
    FOREIGN KEY (idEditora)
    REFERENCES Editora (idEditora)
    ON DELETE NO ACTION -- caso tente deletar, não deixa
    ON UPDATE NO ACTION, -- caso tente atualizar n deixa
...
select * from Genero; -- mostra o conteúdo completo da tabela genero
...
select distinct(Autor.nome) -- produto cartesiano ou join, unindo duas tabelas
...
from Autor, Livro_Autor
where Autor.idAutor = Livro_Autor.idAutor
order by Autor.nome; -- order by ordena pelo nome do autor
...
select Livro.titulo, Genero.descricao, Livro.idEditora as 'Código Editora', Editora.nome
...
--update
update Autor set autor.nome = 'Pedro Lopes'  where autor.idAutor =1;
